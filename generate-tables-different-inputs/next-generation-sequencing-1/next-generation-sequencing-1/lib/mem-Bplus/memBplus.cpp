...
namespace PROJECT_NAME
{
    namespace MOD_NAME
    {
        ...
        void readFile(std::string file, BPlusTree& T)
        {
            ...
            else
            {
                ...
                while (getline(input, b1))
                {
                    ...
                    /*strcpy_s(key.id, strlen(b1.c_str()) + 1, b1.c_str());*/
        ...
        int testFunc()
        {
            PROJECT_NAME::MEM_BPLUS::BPlusTree T;
            PROJECT_NAME::MEM_BPLUS::KeyType X;
            T = PROJECT_NAME::MEM_BPLUS::Initialize();
            std::string path = BPLUES_INIT_FILE_NAME;
            readFile(path, T);

            X.id = 5;
            std::string a1 = quickCheck(T, X);
