...
namespace PROJECT_NAME::MOD_NAME
{
    ...
    void outputFile(std::string QNFilePath,std::string CQFilePath)
    {
        ...
        QNWriteFP.open(QNFilePath,std::ios::out | std::ios::app);

        if(!QNWriteFP.is_open())
        {
            EXCEPTION::printError("File output failed.");
            QNWriteFP.close();
            exit(102);
        }
        ...
        for(auto iter1 : SeqVector)
        {
            ...
            CQWriteFP.open(CQFilePath+std::to_string(index) ,std::ios::out | std::ios::app);
