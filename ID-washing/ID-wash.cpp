...

	{
        ...
        static decltype(auto) thread_deal()
        {
            ...
            for (int i = 0; i < readlength; i++)
            {
                ...
                while (filein.peek() != '\n')
                {
                    std::getline(filein, string_temp, ',');
        ...
        void task() {
            T = Initialize();
            readFile(ID_WASH_INIT_CHARACTERISTIC_FILE_NAME, T);
            filein.open(ID_WASH_INIT_MATCH_FILE_NAME, std::ios::in);
            ThreadPool pool(10);
            pool.init();
            std::vector<decltype(pool.submit(thread_deal))> resultvector;
            std::map<int, decltype(pool.submit(thread_deal).get())> mymap;

            puts("here1");system("pause");
            ...
            puts("here3");system("pause");

            std::ofstream outfile1(ID_WASH_OUTPUT_MATCH_FILE_NAME, std::ios::out | std::ios::app);
            std::ofstream outfile2(ID_WASH_OUTPUT_SEQUENCE_FILE_NAME, std::ios::out | std::ios::app);
            for (auto& element0 : mymap) {
                for (auto& element1 : element0.second) {
                    for (auto& element2 : element1.second) {
                        ...
                        outfile1 << std::endl;
                        outfile2 << element1.first.second + element2.front().second.first << std::endl;
                    }
                }
            }
            outfile1.close();
